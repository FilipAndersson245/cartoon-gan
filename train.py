...
def adv_d_loss(D_real: torch.Tensor, D_fake: torch.Tensor) -> torch.Tensor:
    return D_fake.mean(dim=1) - D_real.mean(dim=1)


def adv_g_loss(D_fake: torch.Tensor) -> torch.Tensor:
    return -D_fake.mean(dim=1)
...
# Load initiated model weights
G.load_state_dict(torch.load("./checkpoints/2020.08.28 12.21_init.pth")["G"])
...
n_batch_batches = min(len(photo_dataloader), len(cartoon_dataloader))

# Endless range loop
for epoch in itertools.count():

    for i, (photo_batch_batch, cartoon_batch_batch) in tqdm(enumerate(zip(photo_dataloader, cartoon_dataloader)), total=n_batch_batches, desc=f"Training epoch {epoch}"):
        ...
        for photo_batch, cartoon_batch in zip(photo_batch_batch, cartoon_batch_batch):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                ...
                loss_D = adv_d_loss(real_pred, fake_pred)
                loss_D = loss_D.sum()
        ...
        with torch.cuda.amp.autocast(enabled=False):
            generated_cartoon_batch = G(photo_batch)
            fake_pred = D(generated_cartoon_batch)
            loss_G = adv_g_loss(fake_pred).sum() + \
        ...
        ###############
        if i % log_interval == 0:
            global_step = epoch * n_batch_batches + i
